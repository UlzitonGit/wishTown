...
public class SecondEyeMovement : MonoBehaviour
{
    ...
    void Update()
    {
        if (player != null)
        {
            ...
            float blendFactor = Mathf.InverseLerp(maxDistance, closeDistanceThreshold, distanceToPlayer);

            Vector3 parallelPosition = new Vector3(
            ...
            Vector3 secondTargetPosition = Vector3.Lerp(parallelPosition, mirroredPosition, blendFactor);

            transform.localPosition = Vector3.Lerp(transform.localPosition, secondTargetPosition, smoothSpeed);
        }
    }
}